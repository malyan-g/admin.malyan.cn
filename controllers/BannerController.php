...
class BannerController extends Controller
{
    ...
    {
        $this->operateModule = OperateLog::EVENT_MODULE_BANNER;
    ...
    {
        $model = $isNewRecord ? new Banner() : $this->findModel();
        
        $request = Yii::$app->request;
        if($request->isPost){
            if($model->load($request->post())){
                // 传到七牛云
                $uploadedFile = UploadedFile::getInstance($model, 'url');
                if($uploadedFile){
                    $model->url = UploadHelper::qnUpload($uploadedFile->tempName, $uploadedFile->extension);
                }
                if($model->validate()){
                    if($model->save(false)){
